...
function initAccounts(){
  ...
  function render(){
    ...
    if(a){
      form.hidden=true; profile.hidden=false; loginTab.hidden=true; registerTab.hidden=true; profileName.textContent=a.username; profileStatus.textContent=L.logged; edit.textContent=L.edit; logout.textContent=L.logout; setAvatar(profileAvatar,a.avatar,a.username); setAvatar(preview,a.avatar,a.username); renderDashboard(a);
    }else{
...
function initCommunityRating(){
  ...
  function refresh(){
    ...
    if(user.options[0])user.options[0].textContent=l.choose; const accounts=getAccounts(); const opts=Object.values(accounts).map(a=>a.username); user.innerHTML=`<option value="" selected disabled>${l.choose}</option>`+(opts.length?opts.map(u=>`<option>${esc(u)}</option>`).join(''):'<option disabled>ابتدا حساب بسازید</option>');
...
.then(d=>{
  ...
  render(); window.__fpAccountRefresh?.(); window.__fpCommentsRefresh?.();
